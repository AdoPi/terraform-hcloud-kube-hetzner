  ...
  k3s-agent-config = { for k, v in local.agent_nodes : k => merge(
    {
      ...
      server    = coalesce(var.control_plane_endpoint, format("https://%s:6443", var.use_control_plane_lb ? hcloud_load_balancer_network.control_plane[0].ip : module.control_planes[keys(module.control_planes)[0]].private_ipv4_address))
